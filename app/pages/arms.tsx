...
import { ArmsWorkouts } from '@/constants/ArmsWorkouts';
...
export default function MenuScreen() {
    ...
    const [armsWorkouts, setArmsWorkouts] = useState<{ id: number; title: string; description: string; image: any; videoUrl?: string; subtitle: string[]; instructions: string | string[]; }[]>([]);
    
    useEffect(() => {
        setArmsWorkouts(armsWorkouts);
    })


    const [isVisible, setVisible] = useState(false);
    const [selectedItem, setSelectedItem] = useState<{ id: number; title: string; description: string; image: any; videoUrl?: string; subtitle: string[]; instructions: string | string[]; } | null>(null);
    // const [formClicked, setFormClicked] = useState(false);

    
   const toggleModal = (armsWorkouts: { id: number; title: string; description: string; image: any; videoUrl?: string; subtitle?: string | string[]; instructions: string | string[]; } | null) => {
        if (armsWorkouts) {
            setSelectedItem({
                ...armsWorkouts,
                subtitle: Array.isArray(armsWorkouts.subtitle) ? armsWorkouts.subtitle : armsWorkouts.subtitle ? [armsWorkouts.subtitle] : [],
            });
        } else {
            setSelectedItem(null);
        }
        setVisible(!!armsWorkouts);
    };
    ...
    const headerComp = <Text style={styles.textHeader}> Arms Workouts </Text>
...
//     overlay: { position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0, 0, 0, 1)'},
//     modalClose: { position: 'absolute', top: 60, right: 40, padding: 10, backgroundColor: 'black', color: 'white', fontSize: 25 },
//     modalCloseText: { color: 'white', fontSize: 25 },
//   });
//   }
