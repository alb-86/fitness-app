...
const navigation = useNavigation<NavigationProp<RootStackParamList>>();
    ...
    const [chestWorkouts, setChestWorkouts] = useState<{ id: number; title: string; description: string; image: any; videoUrl?: string; subtitle: string[]; instructions: string | string[]; }[]>([]);

    useEffect(() => {
        setChestWorkouts(chestWorkouts)
    })
    ...
    const toggleModal = (chestWorkout: { id: number; title: string; description: string; image: any; videoUrl?: string; subtitle?: string | string[]; instructions: string | string[]; } | null) => {
        if (chestWorkout) {
            setSelectedItem({
                ...chestWorkout,
                subtitle: Array.isArray(chestWorkout.subtitle) ? chestWorkout.subtitle : chestWorkout.subtitle ? [chestWorkout.subtitle] : [],
            });
        } else {
            setSelectedItem(null);
        }
        setVisible(!!chestWorkout);
    };
    ...
    const footerComp = <Text style={{color:theme.text, fontSize:15,fontFamily: 'Kalam-Bold', padding: 10}}>&#169; 2025. Powered by ALB Media</Text>
    const headerComp = <Text style={styles.textHeader}> Chest Workouts </Text>
    ...
    return (
        <Container>
            ...
            <TouchableOpacity onPress={goHome} style={styles.homeButton}>
                    ...
                    {/* <Ionicons name="return-up-back" size={60} color="white" style={styles.icon} /> */}
            </TouchableOpacity>
            <FlatList
                scrollEnabled={false}
